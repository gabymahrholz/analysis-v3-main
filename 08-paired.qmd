...
Today, we will focus on the Simon effect. Remember that the Simon effect predicts shorter response times for congruent trials compared to incongruent trials.
...
* **Alternative Hypothesis (H~1~)**: "Response times for congruent trials are significantly shorter than those for incongruent trials in a Simon task." or "Participants will respond significantly faster in congruent trials than in incongruent trials in a Simon task."
...
Challenge yourself! See if you can reproduce the table without hints this time.
...
First, recreate the table described above (i.e., wrangle data into long format). Then, use this table to create an appropriate plot that visualises the differences in mean response times between congruent and incongruent trials."
...
The `cohensD()` function would also take a long format formula approach, such as from `simon_effect_long`, but you would need to assure that the columns are ordered correctly, i.e., Participant 1: condition 1, condition 2; Participant 2: condition 1, condition 2; etc.
...
As with the other t-test, we are conducting a sensitivity power analysis to determine the minimum effect size detectable with the number of participants we have in our sample (n = 160), alpha of 0.05 and power of 0.8. This will tell us if our analysis was sufficiently powered or not.
...
This week's pair-coding activity mirrors the one from @sec-independent. While there’s no lab scheduled this week, I wanted to provide an opportunity for you to practice. You can treat this either as a "Challenge Yourself" activity or get together with friends to work through it together.
