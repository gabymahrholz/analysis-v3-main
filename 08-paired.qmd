...
Today, we'll need the following packages `rstatix`, `tidyverse`, `qqplotr`, `lsr`, and `pwr`. You should have all the packages installed already from previous chapters installed already, but if you need to install any of them, do so via the console (see @sec-install_packages for more detail). 
