...
We are using the package `tidyverse` today, and the data file we should read in today is `dog_data_clean_wide.csv`. I've named my data object `dog_data_wide` to shorten the name.
...
* I've created a new data object `data_bar` but you could also work straight from `dog_data_wide`.
* Consider turning the 2 variables into factors before plotting
* Plotting should be relatively straightforward - default colours and changing the axes labels/ legend title
...
* I've created a new data object `data_vb` but you could also work straight from `dog_data_wide`.
* Consider turning the categorical variable into a factor before plotting
* Plotting tips:
  * the colour scale is one of the viridis options 
  * it's a bit of trial and error for the opacity of the violin and the box width of the boxes 
  * the tricky part might be adjusting the y-axis ticks. Take inspiration from the histogram in @sec-hist
...
Hmmm. We got a plot, but is it the one we aimed for? `r torf (FALSE)`
