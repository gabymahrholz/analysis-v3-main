...
We are using the package `tidyverse` today, and the data file we should read in is `dog_data_raw.csv`.
...
* **Step 1**: Select all relevant columns, such as participant ID and all 8 items of the `Flourishing` questionnaire that participants completed before the intervention. Store them in an object called `data_flourishing`
...
If you want to **challenge yourself** and apply the skills from chapter 2 further, you could wrangle the data from `dog_data_raw` for some of the other questionnaires `pre` and/or `post` intervention:

* the mean score for `flourishing_post` for each participant
* the mean scores for the `PANAS` - Positive and/or negative affect - per participant
* the mean score for the happiness (`SHS`) per participant
